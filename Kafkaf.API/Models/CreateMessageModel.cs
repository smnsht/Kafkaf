...
public record CreateMessageModel(string? key, string? value, Dictionary<string, string> headers)
{
	public byte[]? KeyBytes => string.IsNullOrEmpty(key) ? null : Encoding.UTF8.GetBytes(key);
	public byte[]? ValueBytes => string.IsNullOrEmpty(value) ? null : Encoding.UTF8.GetBytes(value);
